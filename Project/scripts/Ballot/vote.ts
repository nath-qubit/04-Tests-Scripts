...
async function main() {
	...
	// 		: new ethers.Wallet(process.env.VOTER_KEY ?? EXPOSED_KEY);
	
	const wallet = new ethers.Wallet(process.env.VOTER_KEY ?? EXPOSED_KEY);
	...
	const tx = await ballotContract.vote(process.argv[3]);
	console.log(tx);
}
